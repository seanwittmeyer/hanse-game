...
function shipCard(hull,destNm){const cap=HULL[hull].cap;const d=SHIP_DEST[destNm];
  ...
  let rows='<div class="st-trig">'
    ...
    +'<div class="st-seat st-tseat" title="the trigger berth — the last cask loads here and the ship sails at once"><span class="st-num">'+cap+'</span><span class="st-sail">'+LU('sailboat')+'</span></div>'
  +'</div>';
  for(let i=cap-1;i>=1;i--)rows+='<div class="st-berth"><div class="st-seat"><span class="st-num">'+i+'</span><span class="st-ghost">'+LU(QI)+'</span><span class="st-up">'+LU('chevrons-up')+'</span></div></div>';

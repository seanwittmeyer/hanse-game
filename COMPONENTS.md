...
| Component | Count | Size | What it prints |
|---|---|---|---|
| **Recipe cards** | 24 | 1.85 × 2.55 in, single-faced | Six beers × four copies: the beer and its quality, the brew cost, and the exports' fee — Broyhan 1 `H` · Keut 1 `G` · Mumme 1 `G` 1 `H` · Bock 1 `G` 2 `H`. Gruit and Hopped start in hand; three of the four exports are dealt each game, the fourth's cards and tiles to the box. A card crosses the player board, left to right, the first time a cask of its beer boards or is carted. |

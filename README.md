...
> **v0.7 is a ground-up reel-in to *Great Western Trail / Distilled* weight** — *too much game, the right amount of theme.* It keeps the stations, the slots, the dual-role cask, the merchant-shipping fantasy, and the theme, and cuts roughly half the rules (the demand market, the type frontier, Fairs, route lanes, the working-cask Floor, the twins, the recipe-card machinery). See `DESIGN.md` §21.
...
> **✅ v0.7 is complete repo-wide:** the markdown specs **and all five HTML pages** are on v0.7. (The playable `play.html` adds one engine-level relief valve, the **Charter** — a `2 G` single-cask Harbor voyage that keeps the tight slots from deadlocking; see `RULES.md` §5.)

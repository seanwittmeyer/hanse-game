...
- **Non-destructive interaction:** you may **ship a rival's wharf cask** on your ship — they score it & pick its benefit; you get the slot/ship/timing. **Scoring:** delivery value (by destination) + majorities + goals (best few). **End clock:** the shared **Sailed-Ships track fills** → finish the round → score.
...
- `learn.html` (beginner) — **v0.7, current.** `play.html` (reference implementation: DATA → STATE → TURN MACHINE → CELL HANDLERS → SCORING → RENDER), `rulebook.html`, `index.html`, `printables.html` — **still v0.6, port pending** (`DESIGN.md` §21 "Engine deltas" specs the rewrite).
...
5. **`play.html` is mid-port to v0.7.** Until it's rewritten (per `DESIGN.md` §21), the **markdown is canonical**; don't trust `play.html`/`rulebook`/`index`/`printables` for v0.7 rules. When porting, smoke-test headlessly (mocked-DOM `vm`, drive a bot via the rendered buttons' `onclick` strings, assert 2–5p run crash-free to game-over) and bump the save `KEY`.

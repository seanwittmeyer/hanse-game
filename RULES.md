...
Owned ships are consumed when they sail, and the slots are tight, so you always need an escape that can't be locked out. At the **Harbor** you may always **Charter**: pay your **current fare** — **⚙2 `G` +1 `G` per charter you have already taken, capped at ⚙5 `G`** (a fare disc on a small personal track) — to ship **one** Ready cask — from a **vessel** *or* a **slot** — on an **immediate single-cask voyage** to a destination it qualifies for. It **delivers** (scores + benefit, §9) and **advances the Sailed-Ships clock** (§8) like any voyage; it just carries one cask for the fare, so building Cogs/Hulks stays the efficient race. The Charter is the **deadlock guard**: when every slot is full and you own no ship, or a Ready cask clogs a vessel, a charter always frees you (the capped fare stays reachable under the storage cap — the deadlock guard holds; the escalation just stops the relief valve becoming a highway — v0.11 Batch A2, after both human play and the search AI leaned on charters over ships). *(Theme: a 1350 brewmaster who hasn't a hull of their own books space on someone else's.)*
...
- **Charter fare** (⚙2 `G` base, +1 `G` per prior charter, cap ⚙5 `G` — v0.11 Batch A2) and whether it advances the clock — the relief-valve dials. The escalation answers the v26 finding (charters out-competed owning ships); the base must stay cheap enough that the deadlock guard never bites.

...
### Open threads — pick up here next session (as of 2026-06-15, end of the v0.15 batch)
Everything below is **on `main`** (v0.15 "Enshrine" is live repo-wide; `play.html` `KEY = v44`). Nothing is half-finished in the working tree. These are the *deliberately-deferred* items, roughly by priority:
...
6. **Vestigial upgrade-buy path.** `UPGRADE_BUY` + `buyDisplayUp()` still exist in `play.html`, but the UI/docs say upgrades are *earned-only, not for sale* (v0.12.3). The buy path is effectively dead code (the `G` figures are kept only as rarity indicators). *Action:* either remove the dead buy path or document it explicitly so it stops looking like a contradiction.
...
8. **Stale code comment.** `play.html` GM rollout comment (~the `GM_DEEP` block) still says "ship the high end to the Hall" — it now *enshrines*. Trivial wording cleanup.

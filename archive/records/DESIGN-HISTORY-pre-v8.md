# Design history — everything before v8.0

*Cut whole from the root docs on 2026-09-07 when the repo was re-cut (the designer: "anything older than 8.0 belongs in the archive"). `DESIGN.md` at the root carries the v8 story; this file carries what it carried before — the v5.7 architecture as it stood, the v5–v7 change log, the parking lot, the pre-v8 open watches, the v5.3b glossary, and the charter's v5–v7 digest. Reference only; nothing here is live.*

---

## A. The v5.7 architecture, as it stood (from `DESIGN.md` §6)
...
## B. The change log, v0.1 → v7.0b (from `DESIGN.md` §9; newest first)
...
## C. The open watches before v8 (from `DESIGN.md` §10)
...
## D. The v5.3b glossary (from `DESIGN.md` §11)
...
## E. The charter's digest of v5.8, v6 and v7 (from `CLAUDE.md` §7, as it stood 2026-09-07)
...
10–15 turns/seat. The full sheet + the red-team record: `V7-PLAN.md` §10. **The first oracle read of v7.0b (2026-09-02 — a 30-game 2p search-tier corpus, two blind analyses, standing review #4): `archive/records/GATEKEEPER-v70b.md` — verdict PASS-FOR-NOW on the right chassis; the hall is priced dead (314 ⚜ earned, 14 spent), the dice are not the clock (21/30 ceiling endings), one Venture face is the engine; its §8 is the queue.** **THE V8 PROGRAM (designer-ruled 2026-09-04; the review complete and the plan built 2026-09-06): `V8-PLAN.md` §12–§13 — "a good brewer AND a good merchant."** The ruled game in one breath: **twelve dice** (ten in the *personal supply*, one on the *warm Gruit* in vessel 1, one standing at sea as the *starter post*); a die leaves the supply as a **cask** (BREW), a **post** (a die on a *segment* of a lane, climbing +1 per Ship sailing through, no fee) or a **Kontor building** (a die in a Kontor's slot, founded only where your OWN post stands on every segment of its branch from Hamburg; +1 per landing there), and never returns; **the first empty supply ends the game**. **THE QUALITY COUNT: the quality you may deliver anywhere = the number of your dice standing at sea** (global, the starter counts: a Gruit carts to Bruges on turn one, a Bock needs five). **A delivery scores two dice**: the cask's + your building die there; nothing else (no market track, no bands, no premium). Ships 2/3 berths per Kontor + **wild** hulls named by the last loader; nobody owns a hull; sails when full; **the Harbor must commission** and the commission lets you POST on that Ship's lane; its ALTERNATE builds or RAISES at a Kontor. **Bruges by cart only**: *the yard* (3 goods + the only recipe prize; Gruit's only door) or *the hall* (an ⚜ + a Q2+ cask: cask die + **the hall die**, which climbs); **⚜ come only from landing beer at a far Kontor**, no cap. Recipes earned (the yard, the *Gain 1 recipe* tile); the search-brew is the general door to any missing piece (build · recipe · specialist · post …). **No kettle.** Aging = the Cellar + cask bonuses. Gruit fixed (1G → 3 goods). **The wharf's private buildings are a Great Western Trail engine**: four tiles per player, four designs on ANY vacant slot (never over a Work; they fire ON VISIT at the station the slot flanks — designer-ruled 2026-09-06), tier 2 on tier 1 by the FLIP, strictly better, owner-only, goods only, no die. Public Works stay (roster pass after the core); specialists stay as asymmetric powers (Bergen's prize). The end: pips at sea + docked dice at pips + fixed majority pairs + the Flight on beers landed; vessels nothing. **THE BUILD IS LIVE (2026-09-06, the go — `V8-PLAN.md` §12.11):** `RULES.md` v8.0 is composed; `COMPONENTS.md` §0 and `STYLE.md` §4f state the v8 kit and registry; `play.html` is rebuilt in the §13.6 phases (KEY `hanse-v80g` — v8.0b the free-ground ruling, v8.0c *Gain 2 goods, any mix*, v8.0d *at cost*: no building hands out goods — the Granary/Kaufhaus *pay 1 G: Brew once*, the Bonded Store's *pay 1 G: Post on its lane*, the Warehouse vouches (count +1 for a Ship bound there); Gruit's tiles untouched by ruling, v8.0e the *warm Gruit* back (a Ready Gruit in vessel 1, its own twelfth die at 1 on it) and the private tiles' points as the `star-N` glyph, v8.0f the two wild Cogs docked at setup and *one action, one term* (Gain 2 goods · Brew · Age 2 · Load 1; *Source* retired), v8.0g the designer's batch — *deliver · Raise die · Ship* as the registry's words, per-port DIE minimums (London 2 · Bergen 3 · Novgorod 4, read as the cask boards), Kontor building slots 1 / 2 by side, the plan's remaining defaults confirmed; v7 is NOT archived — "it was a bust"); `print.html` (the v8.0 kit — the sea board, the re-faced tiles, the ⚜ tokens and chits, the aid) and `rulebook.html` were re-derived the same day; the art the new faces lack is queued in `art/PROMPTS.md`. Then the designer's own table, then the oracle read; never a corpus before a human table.** The v6
...
## F. The old snapshot row — the v5 line in one breath (from `DESIGN.md` §1 as it stood)

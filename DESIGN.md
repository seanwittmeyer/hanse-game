...
- **THE V8 PROGRAM (2026-09-04 — `V8-PLAN.md`).** The designer ruled v7's core unaddressed: the loop closes on four station verbs, goods are free, aging is time not a spend, recipes are too easy or too hard, ships pay for a public good, buildings are an add-on. v8 re-derives as two loops in tension (the brewer's row, the merchant's row) over ten dice whose every destination is public. Its §7 forks are UNRULED; every watch below stays as history until the designer rules and the paper table plays. **No simulation before that table** (`V8-PLAN.md` §9).

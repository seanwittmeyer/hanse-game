// Headless simulation harness for play3a.html (v3.0-A "One Stop, One Seal" PROTOTYPE).
...
const html = fs.readFileSync(path.join(__dirname, '..', 'play3a.html'), 'utf8');
...
function botMove(p){
  ...
  var ready=readyInVessels(p).length;
  cands.forEach(function(tc){var toll=placing?0:cellToll(tc,p);['row','col'].forEach(function(w){
    ...
    if(ready&&LINES[lk].caps.some(function(cid){return !S.slots[cid];}))v+=1.8;   // v3.0-A: deploy is slot-local — a line needs an open cap slot

...
**Live build: v5.6 “The Glut” (`KEY hanse-v56`, designer-ruled 2026-08-23).** The complete version history and rationale live in **`DESIGN.md` §9** (v5 letters in detail, pre-5.0 as a digest); the live watch-list in **`DESIGN.md` §10**; the consolidated v5 decision record in `archive/records/V5-DECISIONS.md`. Gates at v5.6: verify **391/391** · sim clean (0 crashes / 0 deadlocks) · render-smoke ALL PASS · aid ALL FIT. Pace recovered to **15.3 / 15.0 / 13.6**. **LIVE WATCH:** winner totals rose to **89/89/78** and margins to **15.7–20.8** — the richer opening market (+3) and Novgorod +3★ are unpriced; read at a table before dialing (`DESIGN.md` §10).
...
- **Value lives at the kontore:** the parked die + Bourse (**Novgorod +3★/die**, gate 3+) + tiered majorities (4/2/0 · 5/3/1 · 9/5/2 · 8/5/2) + the bank (Manifest claims — the 12-card demand deck on every non-Bruges hull, one line per delivered cask, each line once per voyage · wharfage · Staple premiums · 1★ bumps) + the Flight (distinct beers SHIPPED, (n−1)² min 3) + the printed end-lines (the Alderman). **End clock:** the first EMPTY TRAY (every die committed) — the dice alone, 13/player ⚙ = THE pace dial; MAX_ROUND 25 backstop; pace target ~12–25 rounds.

...
(v69 → FULL v2.0):** the AI pilots the **Caravan**. v69 wired a selective heuristic (caravan only to **learn a
wanted recipe** or use a **cheap/unshippable** cask) and **fixed a pace bug** — the Caravan was ticking the
Sailed-Ships clock; it is now **clock-neutral** (overland ≠ a sailed ship). **v2.0 (full Option B)** upgraded the
road to a **contested charter board** (a trunk + eastern fork; one first-come charter/town; on-claim boons +
ongoing perks + a road majority), so the AI was re-wired to match: `aiCaravanPick`/`botCaravanPick` now price

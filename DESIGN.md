# Brewhouses of the Hanse — Design (live build v5.1 “Wharf Hands” on the v4.0 spine)
...
- **Slots hold a building and/or a ship — never casks.** A slot = the building (most now
  RIDE the line’s station actions — v5.1) and/or a **load of the ship** docked there (one Ready cask from YOUR vessels; its
...
- **ONE green building family — RIDERS (v5.1)** — nobody owns the use: a rider serves
  whoever’s matching action fires on its line; the builder **stands a quality die on the tile
  at face 1 (v4.9)** — a USED effect turns it up (cap 6), the pips score at game end. **The box
  prints 19 ⚙ — setup deals 17 (v5.1; ≥1 Kiln + ≥1 Mission Quay guaranteed): every wharf a
  different economy. Fees print in grain only — hops pay for USING the power tiles. The event
  family is TWO (Bonded Store · Victualling Yard — both sail away); the Merchants’ Exchange
  (re-manifest ≤2 non-Bruges Ships — v5.0) is the ONE base slot action; the Warping Capstan
  warps docked Ships, cargo riding.** A rider boosts the station action fired on its line —
  primary and alternate alike, once per activation (Granary Source-here-+1 · Mission Quay ×2
  Age-here-+1-step · **Racking Hall** swap-ANY-two-uncapped 3G, the launder engine · **Assay
  House** pay-1H-one-cask-to-Ready · **Abbey Cellar** pay-3H-all-aging-to-Ready · **Hop
  Exchange** pay-≤2H vessel-dice +1 each, Age AND Load · the Capstan warp-first); load-lift
  buildings shape the boarding die / the hull / the load (Malt Kiln +1 · **Tollhouse** −1 for
  +3★ instant · **Bonded Store** +1 and SAILS AWAY with its hull paying contributors 2 goods ·
  Cooperage +1 berth, +1★/load · Customs −2 to the minimum, floor 1 — v5.1 ⚙ · Rich Berth
  pay-2G-fills-the-last-berth, it sails — v5.1 ⚙ · Ropewalk loads-here-+1 — NEW ⚙ · Weigh
  House two-Manifest-lines-per-cask — NEW ⚙). Faucets: London’s prize + the *Gain 1 building*
  bonus — never bought (Scrivener’s Hall · Hiring Post · Almoner’s Stall RETIRED v5.1).
  Overbuild for ONE payment; displaced tile boxed.
- **Everything is earned, not bought** — recipes (Bruges · bonuses), buildings
  (London · bonuses), specialists (Bergen — **per cask, the uniform prize grammar**, v4.7 ·
  bonuses; **15 designs, v5.1 — station superpowers** — the 5 core drips at max(2, n−1)
  copies + the 10 **guild singles** (the Chandler’s swap rides the station Source; the
  Broker and the Brewer’s Mate upgrade your ALTERNATES), two carrying **end-game ★** lines;
  2 seats — the squeeze is picking two of what THIS game shows). The
...
### v5.2 “Groundwork” (2026-08-22 — designer-ruled in-session; the TWO BUILDING FAMILIES)
The designer’s intent, built in one conversation (the record with the rulings verbatim:
`archive/records/V52-GROUNDWORK.md`): *buildings are modifiers to the items in that slot —
a docked ship, a loading cask, maybe the empty slot itself; the line is clear and clean.*
- **PUBLIC WORKS** — the shared family turns **brown** and fully **passive** (no building
  adds an action for anyone; every face fires on its own slot’s traffic). The builder is the
  **investor**: the die ticks on any use and **AT 6 THE INVESTMENT MATURES** (+6★, the die
  home, the tile demolished — churn, runway and the completed metaphor in one rule); an
  overbuild still pays the pips (ruled). Roster 13/9: Kiln ×2 · Tollhouse · **Customs −1**
  (ruled — −2 was “almost broken”: London/Bergen 2+) · **Ropewalk reworked** (ruled — a load
  here also loads 1 onto a DIFFERENT Ship) · Cooperage · Weigh House · **Staple House ×4
  NEW** (*Stapelrecht* — the designer’s destination premium; +2★ ⚙/cask on a matching sail,
  deal 2) · the ephemerals. **Rich Berth CUT** (“meh”, dead through two buffs); Capstan to
  the expansion lot; Granary/MQ/Racking/Assay/Abbey/Hopex/Exchange absorbed into—
- **VENTURES** — the GWT family: an identical **hand of 4 dual-use tiles** per house
  (L1 face from hand / L2 face over your own L1 — one cardboard, never both faces; the
  designer: *“you choose an L1 to skip when you overbuild an L1 just as you skip an L2 by
  building an L1”*). Private, the **owner’s ring**, **no die** (the power is the return),
  never overbuilt by a rival. **THE LADDER (ruled): public → L1 → L2** — an L1 places only
  while one of your dice STANDS on a Public Work (the moment-checked, no-memory form).
  Rack House/Brewery · Counting House/Assay Loft · Factor’s Desk/Staple Rights ·
  Warehouse/Guild Residence (the lone VP face). Owner-only stops carry the action faces —
  everyone else still reads two actions + two loads.
- Engine: the family split (`b.v`/`vAt` · `bKeyAt` blind to ventures) · maturity in
  `bldgTick` · the ladder gates · the dual-use hand (`p.hand`) · `placevent`/`vredeal`
  flows · the Ropewalk chain (`rwChain`) · Staple premiums in `sailShip` · both families in
  the survey verb, the London prize, the AI (priors + MC option spaces) and the render.
  The v5.1 rider plumbing retires whole. KEY `hanse-v52`.
- **v5.3 "The Bourse" (2026-08-22, the third letter — same day):** the beer-value market
  lands — one track (−1…+3 ⚙), a price marker per in-play beer except Gruit & Jopenbier;
  a delivered cask scores **die + track** (ruled); **bulk rise then score** (ruled); brews
  crash their own beer −1; shifts ride Bergen's prize (*specialist + adjust the market by
  1*) and the Ventures' new **public lines** (*"a public action for everyone… the owner
  gets both — that will grease the wheels"*). **Public Works become the start of the
  game** (*"you never put a die on them because they simply start the game"* — 3–4 random
  tiles, rest to the box; the v5.2 investor/maturity grammar retires after one letter) and
  **the open ground** replaces the ladder (an L1 takes any open slot; wharf full → may
  replace a Public Work; never a rival's tile). The Lisboa/Maracaibo thread the designer
  named becomes the game's negative-feedback loop: the loosened economy (Source 3 · no
  dice on buildings) is counterweighted by the market your own volume moves. KEY `hanse-v53`.
- **v5.2b (2026-08-22, the second letter — same day, off the designer's first hands-on
  read):** **THE LADDER IS THE GROUND** — an L1 Venture REPLACES a Public Work YOU
  invested in (pips bank · die home · public tile boxed; never an open slot) — the
  build-then-redevelop arc is the point; the *Brew 1* load bonus takes the **top tile**
  (the search is the station's edge); **Market primary Source 3** ⚙ (*"the game is slower
  because of the lack of resources"* — the first pure faucet widening since v0; sim hook
  `SRCN`); the **Bergen 2-goods fallback** confirmed, evened to 1G+1H and PRINTED (*"a
  competition is good"* — the seat race stays the point). KEY `hanse-v52b`.
- Gates at the ruling: verify **338/338** (the §32 Groundwork battery replaces the rider
  batteries) · sim smokes clean 2–4p (ventures place, climbs land, maturities fire, the
  cross-quay load runs). The full oracle + the balance proposal follow (`archive/records/`).
  Watches: RULES §Open #19 (the have/have-not gap · the prize pull · 4p ground pressure ·
  the no-die runway discount · maturity pacing · STAPLE ⚙ +2/+4 · the greedy venture mix).

### v5.1r — the rider-scope study letter (2026-08-21 — designer-called off GATEKEEPER-v51 item 2; NOTHING ruled)
`KEY hanse-v51r`. The mark re-fund A/B: **RIDER_SCOPE** ⚙ enters the engine (0 = the v5.1
print · 1 = WIDE — a load-bonus Source/Age fired while the rider's verb-hosting line is
active collects that line's riders too; once per activation and authorship hold), sim hooks
`RSCOPE`/`MSPLUS` (utility faces +1, sim-side), builds-by-channel + wide-exposure counters,
the verify §33 battery (**367/367**). The read (300/count × 4 arms + PATHWAYS, all clean —
`archive/records/RIDER-SCOPE-AB-v51.md`): **no arm passes the pre-registered bar** — the wide
window is a structural null (collects ~0.2–0.5/game against 5.9–11.8 bonus fires), faces +1
is noise, the combined arm peaks at 5.4/5.4/4.9 end pips. The reframe: ticks fell ~40% at
v5.1 but **end pips only ~5–10%** — saturation and the printed start faces absorbed it.
...
modify the station actions (superpowers to make the game asymmetric which we already do to a
point) and the buildings are modifiers and less actions to take. I also like the die
manipulation in the buildings.”* The dive’s recommendation accepted as a batch with one
amendment — *“Retire Almoner’s Stall, that was very frustrating for our playtest and it seems
like it can take away from the core of the game bypassing casks to simply place dice out”* —
and one constraint — *“the text on the component tiles needs to be minimal, and utilize the
icons where possible for resources and actions.”* The record: `archive/records/V51-WHARF-HANDS.md`.
- **Stops become RIDERS.** A converted building prints `[host icon] here:` + effect icons and
  boosts the matching STATION action fired on its line — **primary and alternate alike** (the
  placeable, earnable form of the ALT sweep’s echo-strength dial), serving whoever’s action it
  is; each rider fires **once per activation**, and the mason’s die ticks only when the effect
  is actually used. Granary *Source here +1* · Mission Quay ×2 *Age here +1 step* ·
  Racking/Assay/Abbey offered INSIDE the Age allocation · Hop Exchange in Age AND Load ·
  Warping Capstan warp-first on a Load. Scope ⚙: the STATION action only (not load-bonus
  echoes). The **Merchants’ Exchange is HELD one cycle** as the sole base slot action.
- **The shelf turns.** Retired: **Scrivener’s Hall · Hiring Post** (acquisition = Kontore +
  load bonuses, per the manifesto; both on the measured dead/weak shelf) · **Almoner’s Stall**
  (ruled — presence placement flows only through casks: the load bonus, Keut’s perk, the
  champion’s tour; always FREE, the priced channel leaves). Buffed ⚙: **Customs −2** (floor 1;
  the Novgorod die-1 door is the watch) · **Rich Berth** buy-the-empty-berth 2G (the auto
  sail-short retires — dispatch becomes a priced choice). New ⚙: **Ropewalk** (slot loads +1;
  Stevedore stacks to 3) · **Weigh House** (TWO Manifest lines per cask delivered off its
  Ship). Box **19 tiles / 17 designs — deals 17 of 19** (2 sit out; ≥1 Kiln + ≥1 MQ stands).
- **Station superpowers.** The whole roster rewords icon-first as station modifiers;
  **Chandler reworked** — the once-per-turn G↔H swap now rides the station Source;
  **Broker** (1G — your Market ALTERNATE is Source 2) and **Brewer’s Mate** (1H — your
  Brewhouse ALTERNATE searches) trial the alt-upgrade lane; the drips (Braumeister ·
  Innkeeper) stay drips. Roster 13 → **15 designs / 25 tiles** (deck 20/20/25 at 2/3/4p).
- Gates (the full battery, 2026-08-19): `verify-v4` **354/354** (the §32 rider battery new) ·
  sim **300/count 0 crashes/0 deadlocks** — pace **16.6/15.7/14.8** (band **89.7/92.0/87.0%**;
  the A/B v5.0 corpus at c644653: 15.3/14.4/13.8 · 85.3/84.0/80.0 — riders buy ~+1 round,
  DEEPER into the band; the best 4p band on record) · winner totals 55.8/60.0/59.6 (v5.0
  58.5/64.3/63.4) · **the mason’s-mark economy: builds hold (3.0/4.8/5.7/game) but ticks fall
  ~40%** (2.5/5.3/6.9 vs 4.3/7.8/11.4; end pips ~4.5/5.2/4.6 vs ~5.4) — riders fire on the
  station cadence, not every stop; the flagged re-tune candidate (REPORTED, not retuned) ·
  PATHWAYS 100/lane: **the lifter lane runs HOT at 2–3p (61/50%)**, 4p 34/23/20/23 —
  reported · greedy blind spots: Rich Berth buys 0.0 · Capstan/Chandler cold (policy, not
  verdicts — human-table reads) · render-smoke ALL PASS (GM/CM + toggles) · aid ALL FIT.
  Watches live in RULES §Open #17.

### v5.0 “Open Wharf” (2026-08-18 — designer-ruled off the 4p human playtest)
The brief: *“less tight, less a vs. b, more fluid where needed.”* Three deep dives, a critical
self-review, then the ruling verbatim: *“start with item 1. Upon delivery, cask tiles return to
the stack. Then item 2B with the manifest… optional but point bonuses… generic but only apply to
non-bruges ships… a manifest deck which displays 3 beer types/quality requirements/combos each
with a point value. Each only achievable once. Gate remains — ready and quality. Lastly, 3A is
good.”* The record: `archive/records/V5-OPEN-WHARF.md`.
- **Item 1 — census stacks.** The six abstract steer-piles become **real searchable stacks**, one
  per beer, built from the kit census and shuffled at setup (Gruit ×16 · Hopped ×12 · each export
  ×6 · Gose/Duckstein ×8 · Zerbster/Jopenbier ×6; verb mixes per the print offsets). A **full
  Brew searches** the stack and chooses the tile (the station, the `Brew 1` bonus, the brew
  buildings); only the **alternate Brewhouse** takes the top tile blind. **A delivered cask’s
  tile returns to the BOTTOM** (ruled); Tasting pours return the same way; an **empty stack**
  means that beer can’t brew — supply is now a visible, finite, self-refreshing loop.
- **Item 2B — Manifests.** The 15-Order face-up row **retires whole**. A **12-card ⚙ deck** of
  demand cards rides the Ships themselves: every **non-Bruges** hull is dealt one as it enters
  the display (warm-start hulls included; **Bruges sails plain** — its prize is the cargo). A
  card prints **3 generic demand lines** (named starter · quality tier · die face · combo →
  1–4★). At sail, each delivered cask may claim **ONE line it satisfies** — each line **once per
  voyage**, boarding order, ★ banked at once — then the card returns **under the deck pristine**
  (no per-seat record; the component IS the state). Die lines read the **parked face**
  (post-lift, pre-premium); combos read tier AND die on one cask. **Boarding gates unchanged**
  (READY + the Kontor die minimum — “Gate remains”), now co-printed on the trigger berth.
  Forced reworks: **Chronicler** → *“when you claim a Manifest demand: +2★ at once”* ⚙ ·
  **Merchants’ Exchange** → *“re-manifest up to 2 non-Bruges Ships”* ⚙ · hall mode’s ⚜-per-claim
  moves to Manifest claims.
- **Item 3A — primary/alternate stations.** The worker’s own station fires its **PRIMARY**
  action; the line’s other station offers its **ALTERNATE**: Market **Source 2 / Source 1** ·
  Brewhouse **search / top tile** · Cellar **Age 3 / Age 1** · Harbor **Commission / Load 1 onto
  ANY docked Ship** (the Stevedore lifts to 2 per v4.6d). Slots untouched — every activation now
  reads the whole line instead of an either/or.
- **Deferred (ruled):** the buildings pass and the specialists pass wait for the designer
  check-in after this build. Watches live in RULES §Open #16 (pace under primary/alt — the v76
  lesson; Harbor-alt load pressure; stack depletion; search-vs-top; the claim economy).
- Gates (the full battery, 2026-08-18): `verify-v4` **321/321** (batteries rewritten — census
  search/top/bottom-return · Manifest once-per-line/boarding-order/recycle/Bruges-plain ·
  Chronicler +2 in play · Exchange `(kind,id)` re-manifest · primary/alt · a census-vs-kit
  drift gate reading components.js in the vm) · sim **300/count 0 crashes/0 deadlocks**
  (`sim-results-vhanse-v50.txt`) — **pace 15.3/14.4/14.0 · band 85.7/87.0/79.3%**; the A/B vs
  the v4.17b corpus (ed15649, 300/count: 13.3/13.1/12.3 · band 74.3/73.3/59.7%) says the
  primary/alt grammar **slows the game ~+1.5 rounds INTO the band at every count** — the v76
  fear inverts; manifest lines claimed 4.3/6.6/8.9 per game; Novgorod 18.8/20.6/21.4% ·
  ALT sweeps: **0/0** (echoes off) ≈ live pace — the gentle dials aren’t the lever; **2/2**
  (full-strength echoes = the old both-full grammar) reproduces the fast v4.17b pace
  (13.5/13.2/12.8 · band 70.7/77.3/70.0%) — **the echo strength IS the pace dial and 1/1 is
  the print** · PATHWAYS 100/lane clean (3p 38/40/22 — builder coolish; 4p 36/25/20/19 —
  majority warm; REPORTED, not retuned — the majority lane now rides Manifest claims too;
  re-read at the next full battery) · render-smoke ALL PASS (GM/CM + all-toggles arms) ·
  aid ALL FIT.

### v4.17b “Champion’s Tour” (2026-08-15 — study-ruled: the three-round oracle prints)
`TASTING-STUDY-v417.md`, 1,400 CM/GM+chaos games in three rounds, each 200/arm as briefed.
**R1** (base/tast/hot): the cycle runs but starves at the printed numbers (judged 0.67/g) —
and the **hard-category drought is structural**: a bench of three matching Q4+/die-4+ casks
never assembles (dark convened ONCE in 400 games); the rich-prize arm is REJECTED on a
23.5% Novgorod dead-port. **→ the DUEL BENCHES** (dark/export/old/master print bench 2).
**R2** (duel1/duel2): the duels alone barely wake the hard pours — strong seats price a
ripe die’s port alternative (pips + presence + majority + prize) above a contested 7★,
because **the pour forfeits the majority race**; START_INV 2 doubles engagement at no port
cost **→ ruled**. **R3** (tour): **the CHAMPION’S TOUR** — the winning die parks as
presence (face 1) at a delivered-to Kontor of the owner’s choice — returns the forfeited
weight: judging 1.31/g · hard-category wins 2.4× the control · hall★ ~4 · pace on base ·
the CM at 62% (skill likes the lane; the chaos seat neither pays nor collapses). Standing
watches: the slam share (43% — priced self-fills; dial: one die per player per bench) and
**Novgorod dead 20% vs base 10** — the Hall now genuinely bids for ripe dice, which is the
designed tension; the human table rules next. All state remains tiles + dice positions.

### v4.17 “The Tastings” (2026-08-15 — designer-ruled: the hall rethink, Plan B)
The designer’s critique of the shelf Hall, verbatim brief: *it violates the component-state
tenet (“each player can only deliver for each bonus once — how is this tracked?”), and it
is a dumping ground — “easy bonuses from a menu is boring… majorities are fun because they
keep you on your toes.”* Three full plans were drafted (the quality-sorted **Tasting
Bench** · the contest-cycle **Guild Tastings** · the office-ladder **High Table**); the
designer ruled **“Build B.”** The shelf Hall is CUT WHOLE — menus, the once-per-★, pips,
the crown, the first-die engine, the eased deck. **The Guild Tastings:** contest tiles
(deck 12 ⚙ — free/fresh/dark/export/old/master) lie open 2/3/3; **pour** = 1 ⚜ + a matching
Ready cask, its die standing in pour order; **the bench filling IS the judging** (highest
die 1st — the printed ★ + the tile; 2nd 2★ · 3rd 1★; ties → the earlier pour) — the berth
race’s trigger made prestige, with the **door-slam** (filling the last space forces the
count) as the signature move; unconvened benches pay 1★/die at end (the unfilled-hull
grammar); **sets** 2/3 distinct categories +3/+7★. Hall mode plays the BASE Order deck;
claims pay ⚜; the Chancery stands; START_INV=1. All state = tiles + dice positions.
*Engine:* the `tastings` module replaces `hallinv` wholesale (pourDo/judgeTasting/
aiPourBest with win-bid/fill/door-slam arms; TWO MC representative pours per Tasting —
largest + smallest eligible die); sim.js dials STARTINV/TSTARS/BENCH/EJUDGE + the
pours/judged/slams dashboard. *Kit:* the Tastings board + 12 contest tiles; the eased set
deleted. *Gates:* verify **285/285** (the §30 battery rewritten: category law · the
judging · ties · the door-slam · sets · unconvened · the clock · base purity) · sims clean.
*Oracle:* `tasting-probe.js` — 200 CM/GM games/arm + a jittered chaos seat, two rounds
before the printer (the designer’s game-night brief).

### v4.16b “Guild Ledger” (2026-08-14 — designer-ruled: the lane study’s print flips on)
The designer, on the study’s recommendation: *“Make the change flipping the dials if you
think that is the right solution.”* It was — the pooled 1,500-game verdict met every
pre-registered criterion at the cheapest print. **HALL_PIPS = 1**: every die standing on a
shelf **scores its pips at game end** — the Hall is the fifth place a die parks for value,
so volume there is a true lane (committed hall★ ~9.7 vs the ~3.5★ forgone-majority
deficit); the Hall board prints the line, the Final Standing gains a Hall column, the
breakdown modal itemizes shelf dice + the crown. **INV_BLDG = 1**: the **Guild Chancery**
joins the box on the Guildhall sheet (fee 1 `G`, mark start 2, *Gain 1 ⚜ Invitation*) and
is guaranteed into the hall-mode deal (**17 of 21** ⚙) — the renewable-access faucet.
Costs accepted with eyes open: Novgorod dead-port +3 pp (the Reliquary/eastern-run bid,
§Open #13’s watch) · pace +0.2 rounds. NOT printed: the ⚜ cask bonus (the richness dial —
recorded) · the enshrine-count ladder (CUT, two nulls). *Engineering note:* sim.js’s dial
hooks became override-only-if-set — a ruled default must never be silently forced off by an
unset env hook. *Gates:* verify (§31 re-anchored to the ruled defaults) · sim base +
hall-mode + all-toggles · render-smoke ALL PASS.

### v4.16 “Standing Orders” (2026-08-12 — designer-called: the rename + the Hall volume-lane study build)
Two moves in one letter. **(1) Contracts → Orders (RULED):** the designer — *“invites come
with completed contracts which I want to rename Orders.”* Every player surface renames (the
lineage: lading → Contract → Order); the registry’s recorded collision (the old pick had
avoided “Order” because of *load order*) resolves in copy: the tile is the capitalized
**Order**, the sequence prints as **boarding order**. **(2) The volume-lane study build:**
the brief — *“It should be a lane or strategy to go volume there [the Hall]… should
compensate for the lack of majorities points and should have a mechanism to allow continued
access via invites… maybe a bonus on some casks and maybe a new building which gives an
invite on activation.”* The engine gains four sim-tunable dials (all OFF pending the
ruling): **HALL_PIPS** (enshrined dice score their pips at end — the fifth port, the
majority-compensation term) · **HALL_LADDER** (end ★ by enshrine count, Flight grammar) ·
**INV_CASK_W** (the ⚜ *Gain 1 Invitation* load bonus joins hall-mode pile draws) ·
**INV_BLDG** (the **Guild Chancery**: hall-mode building, activate → +1 ⚜; guaranteed into
the deal while on). `invGrant()` routes every faucet and tallies the source split
(order/first/cask/bldg/menu). The **“hall” persona** joins the oracle vocabulary — a
committed-lane MC seat. *Instrument:* `playtests/hall-lane-probe.js` (3p CM · GM ·
GM-hall, seats rotated; per-arm dashboards: lane win rate · hall★ · majority compensation ·
invite flow by source · dead-port · shelf fill). *Gates:* verify **291/291** · sim base +
all-dials **0 crashes/0 deadlocks** · render-smoke ALL PASS. The three-pass study runs at
100 CM/GM games per arm; record: `archive/records/HALL-LANE-STUDY-v416.md`.

### v4.15b “Second Runnings” (2026-08-10, same day — designer-ruled: the expansion pins simplified)
The designer’s read of the v4.14 beers at the table: *“Gose should be gain any 3 goods —
that justifies the 2G cost and makes it a better Gruit when you have the grain. Zerbster
should have Brew Gruit + Free Load as the bonus — right now what you have is confusing,
let’s keep it simple. Duckstein is fine — the race cask.”* Two reprints, one grammar
lesson — **the pinned load bonus IS the beer’s signature; no beer carries a second,
off-tile rule**: **Gose’s pin becomes “Gain 3 goods (any mix)”** (a new bonus magnitude;
the Salt-Trade per-Kontor delivery perk is CUT — `bonusGoods` stays as an empty seam) and
**Zerbster’s pin becomes the compound “Parti-gyle: a free Gruit (a tray die) · Load 1
more”** — the free Gruit offer moves from brew-time (a prompt nobody could place) to the
moment the cask BOARDS, then chains into the extra load; the tray-die price and the
may-decline stand. Duckstein and Jopenbier are untouched (the Jopenbier value worry —
Bock’s one-turn die-6 line vs the four-step climb — is recorded as a designer-facing
question in RULES §Open #14, options open, nothing ruled). *Engine:* `goods3` rides
`enterSource(3)`; `zgyle` reuses the parti UI with a `thenLoad` chain into `enterLoad`;
brewPick’s parti prompt cut. *Gates:* verify **275/275** (the Gose/Zerbster sections
rewritten: goods3 flow · no salt trade · the zgyle chain through the pending-acts pipeline ·
the no-tray-die skip) · sim 3/count base + all-three-toggles **0 crashes/0 deadlocks**.

### v4.15 “Guildhall” (2026-08-10 — designer-ruled: the Hall returns through Contracts)
The tabled Hall comes back as a **Contract-fed shelf board**, an opt-in third toggle — the
designer’s program across three rulings: *(1)* “you need a contract (invitation) to send a
beer to the hall … contracts (invitations) limited and competed for”; *(2)* “an invitation
gained when a contract is earned … expand the pool with easier-accessed contracts … an
invitation for your first appearance on each shelf as a way to make it an engine itself”;
*(3)* “levels 2 and higher have an option of the bonus so there isn’t a race for the best
spot … one is points, the others are actions at no cost … add brew to levels 3 and 4 …
[the old Hall’s] one truly best spot for an early delivery essentially guaranteed the first
player to get ahead — avoid that.” The build: hall mode swaps the Contract deck to an
**eased 20-tile schedule** (1–3★ — the gate must fire repeatedly) and **every claim pays an
⚜ Invitation**; **enshrine** = spend one + a Ready vessel cask (die ≥ shelf) → the die
stands on an **identical** space and the owner picks from the shelf **menu** (Taproom fixed
2★+2 goods · Table 4★/Age 3/Gain 3 · Masters 6★/Brew/Load-1-more · Reliquary 9★/Brew/**the
Guild’s Seal** — claim 1 open Contract outright). **The ★ option is once per player per
shelf** (order-independent — the anti-jackpot structure: first-come races capacity only);
**first die per shelf → +1 Invitation** (four freebies seed the ladder; repeats cost
claims); **crown +6★** for a die on all four shelves. The Hall pays **no majority** — the
lane competes with the majority race for the same ripe dice. It lands on three owned seams:
the `DEST.hall` seam, the Contract row (the Hall’s heir, v4.5b), and the 2026-07-26
dice-sink parking-lot note. **Simulation-tunable by design** (the designer’s ask):
`HALL_SHELVES` is mutable data — sim.js `HALL=1` + `HALL_STARS`/`HALL_MENU` sweep ★ values
and menus, and the usage dashboard prints enshrines-per-shelf, ★-vs-action picks and the
menu-pick histogram (the “which benefits belong” read). *Engine:* the `hallinv` module on
the expansion spine (setup/score/render) + the enshrine flow (`enterEnshrine`/`enshrineDo`/
the Seal) · claimLading pays the Invitation · the deck swap in freshState · greedy policy
(`aiEnshrineBest` vs the line’s stops) + MC options sample every (cask·shelf·option) ·
the Hall panel + acts-bar button. *Kit:* the Guildhall board (letter) · 18 ⚜ Invitation
tiles · the 20-tile set (plotter). *Gates:* verify **271/271** (the 21-check hall battery:
purity off · deck swap · claim→invite · eligibility/menus/once-each★ · the Seal’s
triple-pay · capacity · crown · the tray accounting · the sweep surface) · sim 3/count base
+ 3/count `HALL=1` **0 crashes/0 deadlocks** · a `HALL_STARS`/`HALL_MENU` sweep smoke ·
render-smoke **ALL PASS** incl. three new toggle scenarios (GM/CM with all three on).
*Watches ⚙ (RULES §Open #13):* the Chronicler under the eased deck (reprice before a human
table) · the Seal loop · Age 3 vs the Cellar · P1 tempo (falsify the anti-jackpot claim).

### v4.14 “Beer Atlas” (2026-08-09 — designer-ruled: the expansion beers return)
The two beer toggles come off the v4.0 shelf, **re-derived on the v4 spine** (the Trade Roads
stays tabled). Every signature lands on an existing component instead of a bolted-on
subsystem: **Gose** (Q2 · 2G no-hops · pinned *Gain 2 goods*) carries **Salt Trade** on the
Keut-perk grammar (a styles-keyed delivery perk: +1G +1H, the drips apply) — the relief valve
the binding-hops economy wanted; **Zerbster** (Q3 · 3H no-grain · pinned *Load 1 more*)
carries **Parti-Gyle** as an *optional* free Gruit at brew that costs a **tray die** — the
13-die clock prices the second runnings, better design than the old free value; **Duckstein**
(Q2 · 1G1H · pinned *Place 1 presence*) carries **Smoke-Hardy** as an innate **+1 as it
boards** (the die grammar makes gates-and-value automatic; the old ready-2 collapses to 1 —
the die floors at 1, so the second step never printed) — a recipe-native Novgorod-band
crosser for the cold port; **Jopenbier** re-derives as a **plain Q6** — the dock-vintage rode
the deploy state and is CUT, its jobs re-homed (aging-as-value = four priced steps ·
race-to-end = the empty-tray clock · contestability = the berth race); start 2 → Ready 6,
**6★ anywhere / 8★ Novgorod** (the old `JOPEN_BASE` 8 falls out of the printed premium),
always-acquirable at its formula fee 3H, the sixth Flight type (6 → 25★ — `FLIGHT_PTS[6]`
had waited since v2.2). The deal drafts **3 of 7 with ≥1 of Mumme/Bock guaranteed**. The
v1.9-era blending + thematic buildings stay tabled (the Racking Hall is blending’s heir; the
three buildings duplicated their beers’ signatures). *Engine:* STYLES + the three signature
hooks (deliverCask `bonusGoods` · boardDie/loadCommit `boardLift` · brewPick `partiGyle` +
the `parti` prompt) · `recipeGainable` carries the always-on capstone · dealExports drafts
3-of-7 · the New Game toggles return · greedy teaches (jopen demotion hops-poor; parti at
runway >2) — the MC pair prices everything through `scorePlayer` unaided. *Gates:* verify
**247/247** (the 19-check v4.14 battery: base-off purity ×30 · draft guarantee ×40 · salt
trade + drips · parti take/skip/die-priced · smoke-hardy gate+value+kiln-stack · Q6
start/Cellarman-3/8★-Novgorod/Flight-25 · always-acquirable at 3H + the Scholar waiver ·
toggle-off channel purity) · sim 3/count base + 3/count `EXPANSION=1 JOPEN=1` **0 crashes/
0 deadlocks** (the env arms are new: sim.js reads EXPANSION/JOPEN) · render-smoke ALL PASS.
...
**(1) the rulebook’s art icons print BIG** — the chip comes off wherever the kit has art
(*“right now they are nothing more than a blob of coloured pixels”*); **(2) a named Kontor
wears its OWN crest** on every surface, and the open **Contract row renders the printed tile**
(components.js `ladingTile`), capped at its true 2 × 0.9 in so a wide column can’t blow the row
up; **(3) the heavy paragraphs become DIAGRAMS built from the real components** — Loading,
Sailing & delivery, the Mason’s Mark, in one fixed language (piece → arrow → piece);
**(4) ONE site navigation** (`nav.js`) across all four pages — they had drifted to four
different link sets — with the play app keeping the Player Aid and showing Aid + Rules in its
bar on non-mobile; **(5) the print kit proofs honestly** — the Destinations board’s Contract
slots are cut component-true and centred (they had inherited the building footprint, so the row
measured 7.82″ on a 7.6″ board and ran off its edge), the **Overlay reaches the Kontor parking
fields** (12 mm dice ghosted in), and the **component manifest is a packing list** — quantity ·
a line of theme · where the piece goes — replacing paragraphs that overran the sheet.
Also folded in: the **surface-reconciliation pass** below, which caught the printed **Novgorod
minimum still reading 4+** three letters after v4.10 set it to 3+. *Gates:* verify **228/228** ·
sim 3/count **0 crashes/0 deadlocks** · render-smoke **ALL PASS** · aid **ALL FIT**; every page
checked in a real browser. *The `KEY` bump to `hanse-v413` is the version marker only — it
clears an in-progress game, which is the one real cost of shipping presentation as a version.*
*Lesson recorded:* **every ruling that touches a printed face needs its `COMPONENTS.md` note,
or the kit silently drifts** — that is exactly how the 4+ survived.
...
- **Building** — the ONE green family: **no owner** — serves whoever activates it; the builder
  **stands a quality die on the tile at its printed start face** (v4.9/v4.9b — every use turns it
  ...
  Most are RIDERS (v5.1) — the printed station-icon line boosts that action on the tile’s
  line, once per activation (Granary · Mission Quay · Racking swap · Assay 1H→Ready · the
  Abbey Cellar · the Hop Exchange · the Capstan warp-first); the Merchants’ Exchange is the
  one slot action; load-lift buildings shape the boarding die / the hull / the load
  (Kiln/Bonded +1 die · Tollhouse −1 for +3★ · Cooperage +1 berth, +1★/load · Customs −2
  minimum · Rich Berth 2G-fills-the-last-berth · Ropewalk +1 load · Weigh House 2 Manifest
  lines). The **Bonded Store** is ephemeral — it sails away with its hull (v4.5b).
- **Specialist** — the purple private tile (15 designs — v5.1: the core 5 at max(2, n−1)
  copies + the 10 guild singles; 2 seats), **earned, never bought**: free as Bergen's prize
  (**per cask** — the uniform grammar, v4.7); the
  hire bonus pays the tile's **printed wharf fee** (v4.2/v4.7).

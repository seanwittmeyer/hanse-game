// Headless simulation harness for play3b.html (v3.0-B "The Roster" PROTOTYPE).
...
// and the spoilopt either/or. sim.js stays the canonical harness for the stable play.html.
...
const html = fs.readFileSync(path.join(__dirname, '..', 'play3b.html'), 'utf8');
...
function botVerbValue(p,k){
  if(k==='source')return 1.5+(needShip(p)?0.8:0)+((p.grain+p.hops)<3?0.7:0);
  if(k==='src3g'||k==='src3h')return 1.9+((p.grain+p.hops)<3?0.7:0);
  if(k==='brew'||k==='jbrew')return (openVessel(p)>=0&&p.recipes.some(function(r){return canBrew(p,r);}))?3:0;
  if(k==='send')return sendCasks(p,false).length?4:0;
  if(k==='send2')return sendCasks(p,false).length?4.4:0;
  if(k==='sendv')return sendCasks(p,true).length?4.2:0;
  ...
  if(k==='ageall'){var m2=p.vessels.filter(function(c){return c&&c.step<c.ready;});return m2.length>=2?3:(m2.length?1.5:0);}
  return 0.5;}
function botSlotValue(p,cell){var v=0;var ready=readyInVessels(p).length;
  (STATION_SLOTS[cell]||[]).forEach(function(cid){var t=S.slots[cid];
    if(!t){if(ready)v+=1.4;return;}
    if(t.type==='cask')v+=0.6;
    if(t.type==='ship'&&t.load.length<effCap(t)&&shipLoadableCasks(p).some(function(cs){return canTake(cid,cs);}))v+=1.6;});
  return v;}
function botMove(p){
  ...
  __chosenWhich=which||'recall';doMove(best);
}
function botLine(p){var w=__chosenWhich;   // the verb (or recall) — decided at move time
  if(w==='recall'||rosterUnused(p).indexOf(w)<0){chooseRecall();return;}
  chooseVerb(w);}
function stopPrio(s){
  if(s.kind==='verb'){var vp={source:0,src3g:0,src3h:0,brew:1,jbrew:1,build:0.5,reach:2,agefull:2,ageall:2,send:4,send2:4,sendv:4}[s.k];return vp==null?1:vp;}
...
function botMarket(p){
  ...
  // (v3.0-A: the commission free-load picker is CUT — dockside pickup is the one free load)
  ...
  {var jpref=['crane','cellar','lagering','granary','hopgarden','vessel','quay'];
   for(var ji=0;ji<jpref.length;ji++){var jk=jpref[ji];
     if((S.impDisplay||[]).includes(jk)&&grantableBuy(p,jk)&&canPay(p,IMPROVEMENTS[jk].cost)&&impArea(p)<IMP_AREA_CAP&&p.grain>=(IMPROVEMENTS[jk].cost.g||0)+1){buyImprovement(jk);return;}}}
  cellDone();   // nothing worth acquiring — the Build verb ends (Source is its own tile)
}
...
function botHarbor(p){   // v3.0-A: the unified SEND — pick the cask, then the route
  if(UI.stage==='send_cask'){var refs=sendCasks(p);
    var own=refs.filter(function(r){return refOwner(r)===p.id&&!botJopenHold(r);});
    var pick=null;
    if(own.length){own.sort(function(a,b){return caskEffQ(b)-caskEffQ(a);});pick=own[0];}
    else{var riv=refs.filter(function(r){return refOwner(r)!==p.id;});
      var completing=riv.filter(function(r){return myShips(p).some(function(sid){var sh=S.slots[sid];return canTake(sid,r)&&sh.load.length+1>=effCap(sh);});});
      if(completing.length)pick=completing[0];
      else if(emptySlots().length===0&&riv.length)pick=riv[0];}
    if(!pick){sendDone();return;}
    sendPickCask(pick);return;}
  if(UI.stage==='send_route'){var ref=UI.tmp.sendCask;var rs=sendRoutesFor(p,ref);
    if(!rs.length){sendBack();return;}
    var q=caskEffQ(ref);var d=personaDest(p,q);var isOwn=refOwner(ref)===p.id;
    var hallR=rs.filter(function(r){return r.kind==='hall';})[0];
    if(d==='hall'&&hallR&&isOwn&&!botJopenHold(ref)){sendRoute('hall');return;}
    var ships=rs.filter(function(r){return r.kind==='ship';});
    if(ships.length){var olk=olPersonaKon(p);
      ships.sort(function(a,b){var sa=S.slots[a.slot],sb=S.slots[b.slot];
        if(olk){var ta=(sa.dest===olk)?1:0,tb=(sb.dest===olk)?1:0;if(ta!==tb)return tb-ta;}
        var fa=(sa.load.length+1>=effCap(sa))?1:0,fb=(sb.load.length+1>=effCap(sb))?1:0;if(fa!==fb)return fb-fa;
        return caskValueAt(ref,sb.dest)-caskValueAt(ref,sa.dest);});
      sendRoute('ship',ships[0].slot);return;}
    var chR=rs.filter(function(r){return r.kind==='charter';})[0];
    var jammed=emptySlots().length===0;var noHull=myShips(p).length===0&&!canPay(p,{g:2});
    if(chR&&(jammed||S.ending||noHull)){__charters++;sendRoute('charter');return;}
    if(hallR&&isOwn&&q>=2&&!botJopenHold(ref)&&(jammed||S.ending)){sendRoute('hall');return;}   // drain valve
    sendDone();return;}
  if(UI.stage==='send_charter'){var ref2=UI.tmp.sendCask;var cq=caskEffQ(ref2);
    var ds=DESTS.filter(function(d2){return d2!=='hall'&&cq>=olCharterGate(p,d2);});
    var dd=personaDest(p,cq);if(dd==='hall')dd=null;
    sendCharterDest(dd&&ds.indexOf(dd)>=0?dd:(ds[0]||'bruges'));return;}
  if(sendCasks(p).length){enterSend('stops');return;}
  cellDone();
}
...
function botTap(p){   // v3.0-A: ONE choice — Age the pool, or fit a Specialist
  if(cellarCanAge(p)){cellarAge();return;}
  if(cellarCanImp(p)){var pref=['hopgarden','granary','cellar','vessel','lagering','crane','quay'];
...
function botActOnce(){var p=cur();var U=UI.sub;
  switch(U){
    ...
    case 'spoilopt':{var rd=readyInVessels(p).filter(function(o){return o.c.q>=SPOIL_MINQ;}).sort(function(a,b){return b.c.q-a.c.q;});
      if(rd.length&&emptySlots().length===0)return spoilDeploy(rd[0].i);
      return spoilAct();}

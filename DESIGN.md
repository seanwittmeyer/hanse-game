...
### v0.10 (in review) — every kontor a competitive majority: the laddered majority spread, Bergen normalized (2026-06-08)
A designer call to make the majority game live at **all four kontore**, not just Bergen — i.e. give every destination a **3-tier majority** worth contesting. This is a deliberate *partial revert* of v0.9's headline decision (concentrate the rich majority at Bergen), so the v0.9 warning had to be answered, not ignored: **broad tiered majorities over-feed the volume lean and starve prestige.**
- **The fork (decided with the designer).** Shape = **descending by reach** (richer majority the harder a kontor is to reach) over full parity or "Bergen stays king"; and **normalize Bergen** (it should no longer be the odd benefit-less kontor). Since an *upgrade* benefit would re-clone Bergen with London (the very split v0.9 made), Bergen was normalized with **goods** instead — yielding a clean benefit pairing: **liquidity (Bruges/Bergen → 2 goods)** vs **engine (London/Novgorod → an Upgrade)**.
- **The ladder (sim-tuned).** `DEST.maj` for all four kontore: **Bruges 2/1/1 · London 3/1/1 · Bergen 4/2/1 · Novgorod 5/2/1** ⚙ — 1st-tiers step 2→3→4→5 by reach; 2nd/3rd kept **thin** on purpose. Total pool ≈ v0.9's (26), *not* a balloon — the v0.9 mistake re-created on the first attempt (a 5/7/9/11-style ladder, pool ~58 ≈ 2.2×) shoved volume to **48–64%** and starved prestige/majority, exactly as warned. Shrinking the pool *and* steepening (rich 1st, thin 2nd/3rd) is what restores balance: a broad volume shipper mostly lands **2nd/3rd** at kontore a specialist has locked, so it can't harvest majority points everywhere as a byproduct.
- **Harness fix (load-bearing).** The persona harness's *majority* bot was a v0.9 relic — it camped a single fixed kontor (`__majTarget='bergen'`). In a four-kontor majority world that's not a fair test: the lone majority bot stacks one post while the volume bot **sweeps 1st place at the other three uncontested**, which by itself produced the "volume dominates" reading. Upgraded the bot to **contest the richest reachable majorities** (lock the best by 1st-tier payout, shift to the next once safely ahead by ≥2 presence) — an honest model of a majority player denying the scatter-shipper. Only with this fix does the ladder measure as balanced.
- **Result (PERSONAS, N=500 × 2–5p).** All three leans land near fair at the higher-contention counts: **4p 23/25/27, 5p 21/17/22** (fair 25 / 20); 3p majority runs a touch hot (~38 vs 33); **2p majority hot (~57)** — an expected artifact of 2p skipping 2nd (pure winner-take-all rewards the lock). Prestige is **no longer starved** at any count. Robustness (greedy, N=500 × 2–5p): **0 crashes / 0 deadlocks, ~100% clock-ended, pace 13.8–14.8 (in band)**, seats fair. Saved `playtests/sim-results-v20*.txt`.
- **Engine + verification.** `play.html` `KEY → v20`: `DEST.maj` triples for all four kontore, `grantBenefit` gives Bergen **+2 goods** (the upgrade branch stays London/Novgorod), Bergen's flavor renamed **Monopoly → Bryggen** (it is no longer *the* monopoly majority). `majorityAwards()`/tie/2p-skip logic unchanged (it already read the tier arrays).
- **Still open / next.** The mild 2p/3p majority-lean edge (an artifact of the lone-specialist persona field + 2p winner-take-all) wants a **human playtest** before further tuning — shrinking the ladder more would risk making the four majorities feel un-competitive again, defeating the change's purpose. ⚙ open: whether Novgorod-as-richest-majority over-stacks the premium kontor (it already tops value + grants an upgrade — Q3-gating is the current check); whether Bergen's "Bryggen" goods benefit wants to differ in amount from Bruges to sharpen the liquidity ladder.

# Brewhouses of the Hanse — “Living Slots” reset — PLAN (SHIPPED · live build now v2.3 “Privileges & Works”)

> **Status: SHIPPED — this plan opened as the v1.0 “Living Slots” reset (2026-06-16); v1.7–v2.3 shipped on top, live build now v2.3 “Privileges & Works”.** The
...
> `COMPONENTS.md` / all five HTML pages now on **v2.3 “Privileges & Works”**). This plan replaced the abandoned
...
| **Authorship / engine** | build buildings → run your Floor (§1B) | **your privileges pay you; overbuild pays +3★** *(v2.3 — wharfage retired)* | London/Novgorod earn buildings; the private Floor | investment-heavy, slow to pay |
...
4. **Rebuild `play.html`** to the new rules (sandbox first; keep v0.16.1 live until ready). ✓ shipped — `play.html` is now on **v2.3 “Privileges & Works”** (v0.16.1 archived).
5. **HTML docs** (`learn` / `index`) then **`printables`** last. ✓ shipped — all five pages on **v2.3 “Privileges & Works”** (incl. the alternative card-based `printables2.html`).

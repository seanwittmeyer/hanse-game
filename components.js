...
function caskCardFront(d,act){   // the AGING side: Q·name (+special) · the maturation track · brew cost + the action preview
  ...
  return '<div class="ctile ctA" style="--c:'+d.c+'">'
    ...
    +'<div class="ct-bot">'+(d.tag?'<span class="ct-perk">'+d.tag.split('<br>').join(' ')+'</span>':'')
      +track+'<span class="ct-mini" title="the wharf action on the other side">'+LU(act.ai)+'</span></div>'
  +'</div>';}

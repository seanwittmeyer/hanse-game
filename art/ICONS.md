...
> proper nouns in the subject line. **The `station-harbor.png` re-cut (hull + post die) was
> landed and REVERTED by ruling within the hour — the Round-3 coin stack STANDS.** The die
> read as a component that is not in the action: commissioning pays GOODS (Hulk 1G · Cog
> free), and the coins say "this costs something", which is true. **THE LAW this ruling
> states (standing for every icon): an icon shows the action's OWN components and cost —
> no component in the action, no component in the icon.** If the coin stack ever changes,
> it becomes GRAIN (the actual fee), never a die.

...
const CASKS=[
  ...
  {nm:'Mumme',   c:'#caa12a', q:4, g:1,h:3, n:8,  ready:3, off:1},
...
function buildingCard(d){const foot=(d.verb==='value'?PRIV_FOOT:WORK_FOOT);
  ...
  +'<div class="bt-foot"><span class="bt-eff">'+d.eff+'</span><span class="bt-cost">'+cost(d.g,0)+'</span></div>'
  +'</div>';}
...
function caskCardFront(d,act){   // the AGING side: Q·name (+special) · the maturation track · brew cost + the action preview
  ...
  return '<div class="ctile ctA" style="--c:'+d.c+'">'
    ...
    +'<div class="ct-bot">'+(d.tag?'<span class="ct-perk">'+d.tag.split('<br>').join(' ')+'</span>':'')+track+'</div>'
  +'</div>';}
...
function shipCard(hull,destNm){const cap=HULL[hull].cap;const d=SHIP_DEST[destNm];
  // v3.4b: NO separate head — the TOP berth is the TRIGGER slot and carries the hull's whole identity
  // (hull · commission · kontor · gate). Berths fill BOTTOM → TOP; the info is needed only while the
  // ship loads, and the cask that covers the trigger is the one that sails it (cover-when-obsolete,
  // like the die over the cask's printed Q).
  let rows='<div class="st-trig">'+artLayer('wharf-'+destNm.toLowerCase()+'.png')
    ...
    +'<div class="st-trigrow"><span class="st-bn">'+cap+'</span><span class="st-bhint">last cask \u2192 '+LU('sailboat')+' SAILS</span></div>'
  +'</div>';
  for(let i=cap-1;i>=1;i--)rows+='<div class="st-berth"><span class="st-bn">'+i+'</span><span class="st-bhint">'+LU(QI)+'cask</span></div>';
  return '<div class="stile" style="--c:'+d.kc+';height:'+SHIP_H(hull)+'in">'+rows+'</div>';}

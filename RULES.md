# Brewhouses of the Hanse — Turn & Round Economy (v0.15 — "Enshrine")
...
- **Benefits split into two, by timing (v0.11.2):**
  - **Immediate, paid when a cask is LOADED** (commission free-load, Harbor, or line-fire): the destination's perk — **Bruges/Bergen → 2 goods · London/Novgorod → take an Upgrade from the display.** (Ships sail to kontore only; the Hall is reached by Enshrine, which pays prestige only — §3.) Clean to resolve, one cask at a time. *(You keep this even if the hull never sails — but then the cask scores no points.)*

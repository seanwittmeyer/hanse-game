...
- **ONE green building family** — no owner: every building serves whoever activates it; the
  placer banks **+3★**. **The v4.5b dice pass + the v45d power ladder: 9 of 17 tiles touch a die; fees print in grain only — hops pay for USING the power tiles.** Action buildings
...
- **The Flight** — distinct beers **BREWED** (the flipped recipe cards): (beers−1)², min 3
  (v45h: pure scoring — the slot-unlock duty left with the covers).

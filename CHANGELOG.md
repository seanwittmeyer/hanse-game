...
- **Novgorod now grants a Modifier** (on top of its highest value), addressing the sim's "Novgorod thinnest kontor" signal — the premium Q3 long-haul now pays the most *and* earns a perk. A benefit-aware re-sim shipped **2.4× more casks to Novgorod**. Benefit map: Bruges→goods · London→any Upgrade · Bergen→Modifier + biggest majority · Novgorod→Modifier + top value · Hall→prestige. *(⚙ parked idea: an escalating Charter fare on a track, so chartering-for-upgrades stays a clean opening but spamming the relief valve costs more — left flat at 2 G for now.)*

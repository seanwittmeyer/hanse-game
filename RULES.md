...
- a **commission** (place the **face-up ship of the ship deck** — a neutral hull, destination printed — on any open slot; §5),
...
**Nobody owns a ship.** Each tile is **printed** with its hull (**Cog** cap 2 · **Hulk** cap 3 ⚙) and its **destination** (inheriting that destination's quality minimum), and comes off a shuffled **ship deck** (~20 hulls ⚙ — Bruges/London/Bergen 4 · Novgorod 5 · Hall 3 — sized so the 4–5p clock rarely recycles). A **ship market of 3 ⚙ sits face-up** beside the Market — you **commission any one of the three**, then refill from the deck. *(The display is 3 rather than 1 so a single ill-bound hull can never stall the yard, and commissioning is a real pick — choose the hull bound where **your** cargo wants to go.)*
...
- **Load:** put **one of your own casks** from a slot aboard any ship it qualifies for — via the **Harbor action** (always available) **or** when the **ship's line fires** (a bonus free load). *(You may never load another player's cask — loading is always self-interested.)*
...
- **Benefits split into two, by timing (v0.11.2):**
  - **Immediate, paid when a cask is LOADED** (commission free-load, Harbor, or line-fire): the destination's perk — **Bruges/Bergen → 2 goods · London/Novgorod → take an Upgrade from the display · the Hall → nothing.** Clean to resolve, one cask at a time. *(You keep this even if the hull never sails — but then the cask scores no points.)*
  - **End-game points, sealed only on DELIVERY** (when the ship sails): the cask's **value ★** (incl. the export premium) and its count toward the kontor **majorities**.
- **Sail** (the instant it is **full**): every cask aboard is **delivered** (its points seal). **Filling a ship earns no bonus** — the filler only chose which cask completed it. **Early launch** (Harbor): any player with **at least one cask aboard** sends a part-loaded ship off (delivering their casks for points).
...
- **The interaction:** shared hulls make cargo space and sail timing the contest — racing to fill a rich-bound ship, commissioning where *your* casks (not your rivals') can reach, or launching early to deny a fuller load. *(The old "ship a rival's cask" rule is cut — v0.11; it measured anti-optimal and read as confusing at the table.)*

### Charter — the single-cask relief valve (no tile; an outside hull)
Owned ships are consumed when they sail, and the slots are tight, so you always need an escape that can't be locked out. At the **Harbor** you may always **Charter**: pay the **charter fare** — a **shared market price printed under the Sailed-Ships track**, indexed by voyages delivered ⚙ (per-count rows, e.g. 2p `2·2·3·2·2·4·5·5`; cheap early, dear in the closing stretch; max `5 G`) — to ship **one** Ready cask — from a **vessel** *or* a **slot** — on an **immediate single-cask voyage** to a destination it qualifies for. It **delivers** (scores + benefit, §9) and **advances the Sailed-Ships clock** (§8) like any voyage; it just carries one cask for the fare, so building Cogs/Hulks stays the efficient race. The Charter is the **deadlock guard**: when every slot is full and you own no ship, or a Ready cask clogs a vessel, a charter always frees you (the fare never exceeds ⚙5 `G`, always reachable under the storage cap — the deadlock guard holds; pricing the closing stretch stops the relief valve becoming an endgame highway — v0.11 A2′, after both human play and the search AI leaned on charters over ships). *(Theme: a 1350 brewmaster who hasn't a hull of their own books space on someone else's.)*
...
|---|---|---|---|---|
| **Bruges** (Hub) | Q1 | low (+1) | **5 / 3 / 0** | **gain 2 goods** (liquidity) — takes any beer, the volume on-ramp |
| **London** (Steelyard) | Q2 | mid (+2) | **6 / 4 / 2** | **take 1 Upgrade from the face-up display** (§3) — *the engine destination* (accessible upgrades) |
| **Bergen** (Bryggen) | Q2 | mid (+2) | **richest 10 / 6 / 3** | **gain 2 goods** (liquidity) — its goods are token; the **majority is its draw** |
| **Novgorod** (Long Haul) | Q3 | **high (+4)** | rich **8 / 5 / 2** | **take 1 Upgrade from the display** — premium only; pays the most *and* a perk |
| **The Hall** (Prestige) | Q2 | **prestige ∝ quality (Q×2.5, floored: Q2→5 · Q3→7 · Q4→10 · Q5→12)** | — | *(none)* — out of commerce (the scarcity/Westvleteren lean) |
...
- **The majority is a *big* end-game motivator — every kontor pays a tiered, ranked majority (Lacerda-style):** at each kontor, count delivered casks; **1st/2nd/3rd place take the destination's majority tiers**. **Bruges 5/3/0 · London 6/4/2 · Novgorod 8/5/2 · Bergen 10/6/3** ⚙ — **Bergen is the rich anchor** (its weak goods benefit is offset by the biggest majority); Novgorod rides high for its hard Q3 reach. Delivery *value* is deliberately low (so the points live in the majority race, not in flat per-cask value). **In a 2-player game, 2nd place is skipped** (winner-take-all at each kontor). Tied players split the sum of the tiers they collectively occupy ⚙. Presence at a kontor = your delivered casks there. *(Big majorities tilt scoring toward the kontore, so the **Hall's prestige was bumped to Q×2.5** to keep the volume-vs-prestige axis balanced — sim-validated near fair at 2–5p, `DESIGN.md` §21 v0.10. **Note:** because presence = cask count, **majorities reward shipping wide**, so "go for majorities" is a *volume* play — there is no separate concentrate-on-one specialist that beats it.)*
...
> **Cut in v0.11:** the old "ship a rival's cask" rule (the oracle showed optimal play declines it; the table found it confusing). The interaction now lives **on the shared hulls**: commissioning where *your* cargo goes (and the free berth it grants), the immediate benefit you bank on loading, completing or early-launching a sail to seal your points, and racing rivals for berths on the rich routes — every load is self-interested.

---

## 6. The brewery & upgrades (your private engine)

Your brewery board is private and safe. It holds your **vessels** (start 2, cap 4 ⚙ — throughput), your **recipes**, and your **upgrades**. Upgrades are the engine-building / "get ahead" layer and where strategies diverge. They come from the Market's **face-up Upgrade display** (§3): **buy** one (pay its goods cost) **or earn** one free by delivering to London / Novgorod (§5; Bruges & Bergen hand you goods instead). **Buying is deliberately dear (4–5 `G` ⚙) — about a kontor delivery's worth of goods — so the *deliver → earn an upgrade → brew better* cycle is the privileged path and buying is a costed bootstrap/fallback, not a shortcut.** There are two families — **Rooms** (permanent capability) and **Modifiers** (asymmetric perks).
...
| 8 | **Royal Patent** | your **charter fare is 1 `G` cheaper** (min 1) | `4 G` |
...
- **The Sailed-Ships track fills.** Each voyage (a ship sailing full or launched, **or a Charter §5**) advances the shared, visible **Sailed-Ships track** by one. When it fills, the end is triggered — **finish the round** (equal turns for all) → **score**. It is **self-accelerating** (the more the table ships, the sooner it ends) and the master length dial. Because the shared clock fills *faster* with more players, its length **scales steeply: ~6 / 10 / 13 / 16 slots for 2 / 3 / 4 / 5 players ⚙** (≈ +3–4 per player; a 500-game sim showed the old linear ~6/8/10/12 ended 4–5p games in ~8 rounds, under the target band — and v0.12's variable cask actions stretched rounds-per-voyage enough that the prior 7/11/15/19 let the round ceiling fire, so v0.12.1 trimmed it back to land games on the clock at ~16–18 rounds).
...
- **Warm-start contents** (2 ships dealt from the deck; the Gruit starts Ready in a vessel), the **ship-deck mix** (~20 hulls ⚙ — size, Cog/Hulk ratio, destination spread), the **commission fee** (2 `G` ⚙, with its free berth) vs the **Sailed-Ships length** — the joint pace dials; target first voyage ≈ turn 2–3, a **12–25-round game**. ⚙ watch: shared hulls run charter-heavier at 4–5p than the old owned fleets (the fare row taxes it) — deck mix per count is the ready dial.
- **The charter fare rows** (⚙ per player count, printed under the Sailed-Ships track — v0.11 A2′; 2p `2·2·3·2·2·4·5·5`) and whether a charter advances the clock — the relief-valve dials. Tune the rows to *when* charters should be welcome (cheap stretches) vs taxed (the closing rush); the fare answers the v26 finding that charters out-competed owning ships, and its `5 G` cap keeps the deadlock guard safe.
...
- **The shared-hull interaction (v0.11.2):** whether commissioning's free load is too strong (it bypasses deploy and banks the benefit early), whether benefit-on-load over-feeds upgrades/goods, and whether ships are *sailed* enough vs charters at the table — the live interaction dials.
